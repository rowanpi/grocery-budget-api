...
class MediaService:
    ...
    def upload_image(self, file, filename, description=None):
        ...
        if settings.grocery_budget_image_store == 'local':
            os.makedirs(os.path.dirname(settings.grocery_budget_image_base_path), exist_ok=True)
            if file.content_type == 'image/jpeg':
                file.content_type = 'image/jpg'  # TODO this is a bit of hack to make sure the extension is .jpg and not .jpe

            if not settings.grocery_budget_image_base_path.endswith('/'):
                settings.grocery_budget_image_base_path += '/'

            file_path = settings.grocery_budget_image_base_path + filename
            with open(file_path, 'wb') as f:
                f.write(file.file.read())
            image = CreateImage(
                store=settings.grocery_budget_image_store,
                address=file_path,
        ...
        elif settings.grocery_budget_image_store == 's3':
            ...
            raise UnsupportedImageStoreException("Unsupported image store: " + settings.grocery_budget_image_store)

        image_entity = self.media_factory.create_image_entity_from_image(image)
        image_entity = self.media_DAO.add_image(image_entity)
        return image_entity

    # create image is intended to be called by the media routers as it returns the image model, not the entity. Upload
    # image returns the entity and is to be called by another service
